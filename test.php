...
$moon_image = load_katalog($json_dir . '/lunar_ephemeris.json');
...
<body>
  ...
  <!--  svg moon phase legend  -->

  <?php
  $startMinutes = 720;  //začneme v poledne
  
  $graph = $moon_image['altitude_graph'] ?? [];
  // -----------------------------
  ...
  <svg viewBox="0 0 <?= $width ?> <?= $height ?>">
    ...
    <?php for ($i = 1; $i <= $yLines; $i++): // nezačínáme od 0°, aby nebyla falešná čára ?>
      ...
      $gy = $paddingTop + $innerH * (1 - ($altVal / $maxAlt));
      ?>
      <line x1="<?= $paddingLeft ?>" y1="<?= $gy ?>" x2="<?= $width - $paddingRight ?>" y2="<?= $gy ?>" stroke="#333"
        stroke-width="1" />
    ...
    <?php for ($i = 0; $i <= $xSteps; $i++):
      ...
      $isLabel = ($minute === 0) && ($hour % 2 === 0);
      ?>
      <line x1="<?= $vx ?>" y1="<?= $paddingTop ?>" x2="<?= $vx ?>" y2="<?= $height - $paddingBottom ?>" stroke="#333"
        stroke-width="1" />
    ...
    <line x1="<?= $paddingLeft ?>" y1="<?= $paddingTop ?>" x2="<?= $paddingLeft ?>" y2="<?= $height - $paddingBottom ?>"
      class="axis" />
    <text x="<?= $paddingLeft - 35 ?>" y="<?= ($height / 2) ?>" class="text-small" text-anchor="middle"
    ...
    <?php if ($transitX !== null): ?>
      <line x1="<?= $transitX ?>" y1="<?= $paddingTop ?>" x2="<?= $transitX ?>" y2="<?= $height - $paddingBottom ?>"
        stroke="red" stroke-width="1.5" />
    <?php endif; ?>
  </svg>
