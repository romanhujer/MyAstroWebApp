...
if ($total_rows == 0)
    die("Pro zadaný interval nejsou data.");
...
function generate_svg_line_chart($data, $field, $width = 900, $height = 220, $color = "#ff0000", $label = "")
{
    ...
    // fyzikální limity
    if ($field === 'tlak_msl' && $min_val < 800)
        $min_val = 800;
    if ($field === 'vlhkost' && $min_val < 10)
        $min_val = 10;
    ...
    <form method="get">
        <label>Od:
            <input type="datetime-local" name="from"
                value="<?php echo htmlspecialchars($from_input_value, ENT_QUOTES, 'UTF-8'); ?>">
        </label>
        <label>Do:
            <input type="datetime-local" name="to"
                value="<?php echo htmlspecialchars($to_input_value, ENT_QUOTES, 'UTF-8'); ?>">
        </label>
        <label>Tlak:
            <input type="hidden" id="p" name="p" value="no" />
            <input type="checkbox" id="p" name="p" value="yes" <?php if ($presure === 'yes'): ?> checked <?php endif; ?> />
        </label>
        <label>Vlhkost:
            <input type="hidden" id="h" name="h" value="no" />
            <input type="checkbox" id="h" name="h" value="yes" <?php if ($humidity === 'yes'): ?> checked <?php endif; ?> />
        </label>
